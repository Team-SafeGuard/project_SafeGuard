...
**AI-YOLO** 서비스는 사용자가 업로드한 민원 이미지를 분석하여 **민원 유형(Type)**과 **담당 기관(Agency)**을지 자동으로 추천해주는 **객체 탐지(Object Detection)** 모듈입니다.
...
- **이미지 휘발성**: 분석 직후 서버 내 임시 파일(`temp_*.jpg`)을 즉시 삭제(`os.remove`)하여 개인 프라이버시 침해 우려 제거.
